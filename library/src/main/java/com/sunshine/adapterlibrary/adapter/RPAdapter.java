 ...
 */
public class RPAdapter<T> extends Adapter<RViewHolder> implements RVBAdapter<Adapter<RViewHolder>> {
    ...
    private PConverter<? super T> mPConverter;
    ...
    public void onBindViewHolder(RViewHolder holder, int position) {
        if (position >= mHeaderViews.size() && position < mHeaderViews.size() + mList.size())
            mPConverter.convert((RViewHolder) holder, (T) mList.get(position - mHeaderViews.size()), position - mHeaderViews.size());
    }
    ...
    public RViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (type) {
            ...
            default:
                return RViewHolder.get(mContext, parent, mItemLayoutId, -1);
